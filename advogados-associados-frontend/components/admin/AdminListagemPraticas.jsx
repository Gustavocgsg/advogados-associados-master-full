...
export default function AdminListagemPraticas() {
    ...
    async function loadingPractices(){
    ...
    async function removePratice(e,id){
        e.preventDefault() //inibe o comportamento padrão da tag  "a"
        const response = await axios.delete(URL_API + '/practices/'+ id) //requisição de deletar vinda do adones que executa a remoção no banco
        await loadingPractices()
            ...
            <div className="column">   
                <div className="p-1 mx-5">
                    ...
                    <div className="table-container">
                        <BtnAdicionar />
                        <Menssagem close={ closeMsg } visible={ msgVisible }>Prática Removida com sucesso !</Menssagem>
                        <table className="table" width="100%">
                            <TableTitle />
                            <tbody>
                                <TableRow deleteRow={ removePratice } list={ list }/>
                            </tbody>
                        </table>
                    </div>
                </div>
...
function Titulo(props){
    return(
        <div className="columns is-variable is-desktop">
            <div className="column">
                <h1 className="is-size-1">Práticas Juridicas</h1>
                <h2 className="is-size-4">Listagem de Práticas</h2>
            </div>
        </div>
    )
}
...
function BtnAdicionar(props){
    ...
    <div className="field is-grouped">
        <div className="control">
            <Link href="/admin/cadastroPratica"><button className="button is-primary is-focused">Adicionar Prática</button></Link>
        </div>
    </div>        
    )
}
                ...
                function(item){
                    return(
                        <tr key={ item.id }>
                            ...
                            <td><Link href={ "/admin/cadastroPratica?id=" + item.id }><a href="#"><i className="fas fa-pencil-alt"></i><span> Alterar</span></a></Link></td>
