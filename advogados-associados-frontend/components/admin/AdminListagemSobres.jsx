...
export default function AdminListagemSobres() {
    ...
    async function removeAbout(e,id){
        e.preventDefault() //inibe o comportamento padrão da tag  "a"
        const response = await axios.delete(URL_API + '/abouts/'+ id) //requisição de deletar vinda do adones que executa a remoção no banco
        await loadingAbouts()
            ...
            <div className="column">   
                <div className="p-1 mx-5">
                    ...
                    <div className="table-container">
                        <BtnAdicionar />
                        <Menssagem close={ closeMsg } visible={ msgVisible }>Sobre Removido com sucesso !</Menssagem>
                        <table className="table" width="100%">
                            <TableTitle />
                            <tbody>
                                <TableRow deleteRow={ removeAbout } list={ list }/>
                            </tbody>
                        </table>
                    </div>
                </div>
...
function Titulo(props){
    return(
        <div className="columns is-variable is-desktop">
            <div className="column">
                <h1 className="is-size-1">Sobre</h1>
                <h2 className="is-size-4">Listagem de Sobre</h2>
            </div>
        </div>
    )
}
...
function BtnAdicionar(props){
    ...
    <div className="field is-grouped">
        <div className="control">
            <Link href="/admin/cadastroSobre"><button className="button is-primary is-focused">Adicionar Sobre</button></Link>
        </div>
    </div>        
    )
}
                ...
                function(item){
                    return(
                        <tr key={ item.id }>
                            ...
                            {/* <td>
                                ...
                                <div className="modal-content bg-cover">
                                    <span className="image is-4by3">
                                        <img src={ URL_API + '/abouts/images/' + item.fileimg } alt="Foto da notícia" />
                                    </span>
                                </div>
                                <button onClick={ () => showimage(item.id) } class="modal-close is-large" aria-label="close"></button>
                                </div>                            
                                <img onClick={ () => showimage(item.id) } src={ URL_API + '/abouts/images/' + item.fileimg } alt="Foto" width="170px" srcset=""/>
                            </td> */}
                            ...
                            <td><Link href={ "/admin/cadastroSobre?id=" + item.id }><a href="#"><i className="fas fa-pencil-alt"></i><span> Alterar</span></a></Link></td>
